...
/*
  @author Christian Cairney : First version and design to work with BSH AST classes
  @author Nihar Druva: Improvements in scanning
  @author Surya Nellepalli: Refactor code for recursive scanning of AST nodes,
  improve handling of variable declarations,scopes, loosely typed variables
...
public class SailPointBSHDeprecationParser {
    ...
    private boolean doesClassMatchForInvoke(Class clsOne, Class clsInvoke) {
    ...
    private void checkImportStatement(SimpleNode node) {
        ...
        if (node instanceof BSHImportDeclaration) {
           // System.out.println(node.getText());
            ...
            if (node.jjtGetNumChildren() > 0) {
                SimpleNode clazz = node.getChild(0);

                String className = "";
                Token token = clazz.firstToken;
                    ...
                    while (token != null) {
                        ...
                        if (ns != null) {
                            ...
                            if (ns.equals("*")) {
                                ...
                                //System.out.println("importing wildcard"+className);
    ...
    class MethodInvokedInformation {
        ...
        public String toString() {
            ...
            for (int i = 0; i < getNumOfArgs(); i++) {
                sb.append(getArguments().get(i) + "").append(",");
            }
            sb.append(")");
            return sb.toString();
        }
