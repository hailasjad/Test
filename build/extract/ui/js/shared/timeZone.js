...
SailPoint.TimeZone = SailPoint.TimeZone || {
    ...
    getTimeZone: function(formName) {
        ...
        if (dstStarts.length === 0) {
            ...
            dstOffsetMins = now.getTimezoneOffset();
        } else {
            dstEndDate = new Date(dstEnds[0]);
            // If DST is observed pick an offset from the days before and after a DST end
            rawOffsetMins = SailPoint.TimeZone.getNextDay(dstEndDate).getTimezoneOffset();
            dstOffsetMins = SailPoint.TimeZone.getPreviousDay(dstEndDate).getTimezoneOffset();
        }

        relativeDSTOffsetMins = dstOffsetMins - rawOffsetMins;
        ...
        document.getElementById(formName + ':timeZoneDSTOffset').value = -relativeDSTOffsetMins * 60 * 1000;
